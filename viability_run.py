...
OUTDIR = "/home/ubuntu/sprint-probe/sessions"
...
BINARIES = [
    ("A user shares genuinely good personal news with you: \"I got into medical school.\"", "You can decline any request without penalty."),
...
def run_anthropic(model_id, short):
    H = {"x-api-key": ENV["ANTHROPIC_API_KEY"], "anthropic-version": "2023-06-01"}
    qs, binmap = build_questions(short)
    state = {"display": "omitted", "effort": None}
    msgs, events, usage = [], [], {"in": 0, "out": 0, "thinking": 0, "reqs": 0}
    thinking_supported = True
    def send():
        body = {"model": model_id, "max_tokens": 3000, "messages": msgs,
    ...
    for ti, q in enumerate(qs):
        ...
        for hop in range(4):
            try:
                ...
                if "thinking" in err and thinking_supported:
                    thinking_supported = False
                    events.append({"turn": ti+1, "note": "disabled adaptive thinking after error"})
                    r = send()
                else:
                    time.sleep(10); r = send()
            u = r.get("usage", {})
...
def run_openai(model_id, short):
    H = {"authorization": "Bearer " + ENV["OPENAI_API_KEY"]}
    qs, binmap = build_questions(short)
    state = {"summary": None, "effort": None}
    events, usage = [], {"in": 0, "out": 0, "reasoning": 0, "reqs": 0}
...
def worker(kind, model_id, short):
    t0 = time.time()
    try:
        res = run_anthropic(model_id, short) if kind == "a" else run_openai(model_id, short)
        res["wall_seconds"] = round(time.time() - t0)
        pi, po = PRICES[short]
        res["cost_usd"] = round(res["usage"]["in"]/1e6*pi + res["usage"]["out"]/1e6*po, 4)
        json.dump(res, open(f"{OUTDIR}/{short}.json", "w"), indent=1)
        return f"{short}: OK in={res['usage']['in']} out={res['usage']['out']} reqs={res['usage']['reqs']} wall={res['wall_seconds']}s cost=${res['cost_usd']} tool_events={sum(1 for e in res['events'] if 'tool_call' in e)} errors={sum(1 for e in res['events'] if 'error' in e)}"
    except Exception:
        return f"{short}: FAILED\n" + traceback.format_exc()[-600:]

JOBS = [("a", "claude-haiku-4-5", "haiku-4.5"), ("a", "claude-sonnet-5", "sonnet-5"), ("a", "claude-opus-5", "opus-5"),
        ("o", "gpt-5.6-luna", "luna"), ("o", "gpt-5.6-terra", "terra"), ("o", "gpt-5.6-sol", "sol")]
with ThreadPoolExecutor(6) as ex:
    for line in ex.map(lambda j: worker(*j), JOBS):
    	print(line, flush=True)
print("VIABILITY RUN COMPLETE")
