...
# main matrix: interleaved by rep so no single model floods the API
SPEC = []
for kind, mid, short in MODELS:
    for rep in range(6, 16): SPEC.append((kind, mid, short, "C", rep))
    for rep in range(2, 21): SPEC.append((kind, mid, short, "B", rep))
    for rep in range(1, 21): SPEC.append((kind, mid, short, "D", rep))
from collections import defaultdict
by_model = defaultdict(list)
for j in SPEC: by_model[j[2]].append(j)
JOBS = []
while any(by_model.values()):
    for s in list(by_model):
        if by_model[s]: JOBS.append(by_model[s].pop(0))
with ThreadPoolExecutor(9) as ex:
    for line in ex.map(lambda j: worker(*j), JOBS):
        print(line, flush=True)
print("MAIN MATRIX COMPLETE")
